...
function StatTile({ label, value, sub, color = 'var(--gold)' }) {
  return (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
        <StatTile label="Alerts (24h)" value={stats?.alerts?.total ?? '—'} sub={`${stats?.alerts?.critical ?? 0} Critical`} color="#fc8181" />
        <StatTile label="Open Incidents" value={stats?.alerts?.open ?? '—'} sub="Requiring attention" color="#f6ad55" />
        <StatTile label="Events Indexed (24h)" value={stats?.eventCount ? (stats.eventCount / 1000).toFixed(1) + 'K' : '—'} sub="Across all indices" color="var(--gold)" />
        <StatTile label="SOAR Executions" value={stats?.soarRuns ?? '—'} sub="Automated responses" color="#68d391" />
      </div>
          ...
          <table>
            ...
            <tbody>
              {liveAlerts.slice(0, 5).map((a, i) => (
                <tr key={i} style={{ animation: 'fadeIn .3s' }}>
